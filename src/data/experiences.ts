...
export const experiences: ExperienceProps[] = [
  {
    ...
    description: [
      "Owned end-to-end design and implementation of Application Control Posture Check, a novel internal DAST tool that scans apps against GRC controls, cutting manual review time from 3 hrs to a 5-min automated scan; demoed at a local OWASP chapter",
      "Built the scanning engine in Python, provisioning containers via Terraform to run compliance checks across apps in parallel",
      "Engineered the engine as a modular, strictly-typed package with pluggable auto-discovered rules, swappable identity providers (Okta, Idira) behind a vendor-agnostic factory, and typed REST/SQL/DNS clients enforced at Pydantic boundaries",
      "Automated 16 GRC controls (NIST CSF) across 10+ apps — SSO, privileged access, service accounts, X.509, DMARC — replacing one-time onboarding checks with continuous daily scans",
      "Built both ends of the risk dashboard data flow: a pipeline pushing scan findings into BigQuery, and a Django REST API the dashboard queries to pull them back, giving the CISO and Board live security posture and control-drift visibility",
      "Architected and coded the risk dashboard’s first RBAC system, giving each InfoSec team least-privilege access to its own data; audited and remediated 10+ flaws including SQL injection",
    ],
